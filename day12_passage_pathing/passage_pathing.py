...
def traverse_node(node, visited):
    if 'end' not in node:
        print(node)
        print([path for path in rows if (any(n in path for n in node)) and (path != node)])
        possible_paths = [path for path in rows if (any(n in path for n in node)) and (path != node)]
...
def find_n_paths(rows, visited_small):
    ...
    path_nodes = [path for path in rows if path not in start_nodes and path not in end_nodes]
