...
def flash(updated_positions, flashed):
    flashing = [
        ...
        if value > 9 and key not in flashed
    ]
    ...
    if flashing:
        ...
        for key in flashing:
            ...
            for adjacent in adjacents_cells:
                ...
                if (all(i >= 0 for i in adjacent_pos)) & (
                    all(i < 10 for i in adjacent_pos)
                ):
...
def update_step(positions: dict) -> dict:
    ...
    post_flash_positions.update(
        {p: 0 for p, val in post_flash_positions.items() if val > 9}
    )
